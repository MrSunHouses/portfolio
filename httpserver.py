...
def main():
    ...
    while True:
        ...
        with(connection_to_browser):
            ...
            file_name,response_body, dict = hande_special_routes(file_name, post_data)
            ...
            with(writer_to_browser):
                try:
                    ...
                    dict = {}
                    dict["error_message"] = error_string
                    response_body = bytearray("error", encoding="utf-8")
...
def get_response_body(file_name,dict=""):
    # TODO add more fun stuff here <3
    with(open(file_name, "rb") as fd):
        response_body = bytearray(fd.read())
        #if there is a dictionary use it to inject variables values
        if dict !="":
            html_contents = response_body.decode("utf-8")
            html_contents = html_contents.format(**dict)
            response_body = html_contents.encode("utf-8")

    return response_body

def write_to_browser(writer_to_browser,content_type,response_body):
...
def hande_special_routes(file_name, post_data):
